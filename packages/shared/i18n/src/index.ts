export const clear_filters = "Clear filters";
export const create_another = "Create another";
export const created_at = "Created at";
export const date_range_custom = "Custom date range";
export const date_range_last_30_days = "Last 30 days";
export const date_range_last_7_days = "Last 7 days";
export const date_range_last_90_days = "Last 90 days";
export const date_range_last_day = "Last day";
export const date_range_last_hour = "Last hour";
export const date_range_next_30_days = "Next 30 days";
export const date_range_next_7_days = "Next 7 days";
export const date_range_next_90_days = "Next 90 days";
export const date_range_next_day = "Next day";
export const date_range_next_hour = "Next hour";
export const date_preset_today = "Today";
export const date_preset_tomorrow = "Tomorrow";
export const date_preset_1_week = "1 week";
export const delete_all = "Delete all";
export const delete_selected = "Delete selected";
export const delete_task = "Delete task";
export const description = "Description";
export const due_date = "Due date";
export const error_message_fallback = "No further information is available.";
export const error_name_fallback = "An unknown error occurred";
export const filter_by_due_date = "Filter by due date";
export const filter_by_status = "Filter by status";
export const filter_tasks = "Filter tasks";
export const filters = "Filters";
export const home = "Home";
export const includes = "includes";
export const is = "is";
export const new_task = "New task";
export const overdue = "Overdue";
export const priority = "Priority";
export const priority_high = "High";
export const priority_low = "low";
export const priority_med = "Medium";
export const priority_urgent = "Urgent";
export const set_priority = "Set priority";
export const status = "Status";
export const status_completed = "Completed";
export const status_in_progress = "In progress";
export const status_to_do = "To-do";
export const submit = "Submit";
export const task = "Task";
export const task_details = "Task details";
export const tasks = "Tasks";
export const tasks_table = "Tasks table";
export const title = "Title";
export const try_again = "Try again";
export const upcoming = "Upcoming";
export const update = "Update";
export const updated_at = "Updated at";
